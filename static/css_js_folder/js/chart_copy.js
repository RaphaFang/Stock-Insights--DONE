document.addEventListener("DOMContentLoaded", function () {
    ...
    options: {
      scales: {
        y: {
          type: "linear",
          position: "left",
          min: undefined, // 自动调整最小值
          max: undefined, // 自动调整最大值
          title: {
            display: true,
            text: "Size",
          },
  ...
  ws.onmessage = function (event) {
    ...
    // 记录数据处理的每一步
    console.log("Parsed time label:", timeLabel);
    ...
    // 确保 price_change_percentage 不为 undefined
    if (incomingData.price_change_percentage !== undefined) {
      const color = incomingData.price_change_percentage > 0 ? "rgba(255, 0, 0, 1)" : "rgba(0, 128, 0, 1)";
      priceColors.push(color);
    }
